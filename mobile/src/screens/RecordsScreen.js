...
export default function RecordsScreen() {
  ...
  if (loading) {
    return <View style={s.center}><ActivityIndicator size="large" color="#5b8ff9" /></View>;
  }
  ...
  return (
    <View style={s.container}>
      ...
      {items.length === 0 ? (
        <Text style={s.empty}>No records or files found.</Text>
      ) : (
        <FlatList
          data={items}
          ...
          renderItem={({ item }) => {
              ...
              return (
                <View style={s.card}>
                  <View style={s.cardHead}>
                    ...
                    <Text style={s.cardDate}>{new Date(r.timestamp).toLocaleDateString()}</Text>
                  </View>
                  {r.data?.diagnosis ? <Text style={s.cardText}>Diagnosis: {r.data.diagnosis}</Text> : null}
                  {r.data?.notes ? <Text style={s.cardText}>Notes: {r.data.notes}</Text> : null}
                  <Text style={s.cardHash}>Block: {r.hash?.substring(0, 16)}...</Text>
                </View>
              );
            }
            const f = item.data;
            return (
              <View style={s.card}>
                <View style={s.cardHead}>
                  <Text style={s.cardAuthor}>{f.originalname}</Text>
                  <Text style={s.cardDate}>{new Date(f.timestamp).toLocaleDateString()}</Text>
                </View>
                <Text style={s.cardText}>Type: {f.mimetype}</Text>
                <Text style={s.cardText}>Size: {(f.size / 1024).toFixed(1)} KB</Text>
              </View>
            );
          }}
        />
